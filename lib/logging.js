 ...
 * MEJORADO: Formato compacto de una línea para facilitar copia
 */
function formatLog(level, message, data = null) {
  const timestamp = new Date().toISOString();
  
  // Formato compacto: todo en una línea, JSON sin espacios
  const logEntry = {
    ts: timestamp,
    level: level,
    msg: message
  ...
  if (data && typeof data === 'object') {
    // Mergear data en el objeto principal para formato más compacto
    Object.assign(logEntry, data);
  } else if (data !== null) {
    logEntry.data = data;
  }
  
  // Retornar como JSON compacto (sin espacios) para fácil copia
...
function error(message, data = null) {
  if (logLevelValue >= LOG_LEVELS.ERROR) {
    const logLine = formatLog('ERROR', message, data);
