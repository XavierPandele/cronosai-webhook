...
function generateMarkdownConversation(state) {
  ...
  if (repeatedMessages.length > 0) {
    ...
    suggestions.push(`💡 **Solución**: Gemini ya genera variaciones automáticas, revisar prompts`);
    suggestions.push(`💡 **Mejora**: Añadir más ejemplos variados en los prompts de Gemini`);
  }
  ...
  if (errorMessages.length > 0) {
    ...
    if (unclearResponses.length > 0) {
      ...
      suggestions.push(`💡 **Solución**: Mejorar prompts de Gemini para extracción más robusta`);
      suggestions.push(`💡 **Técnica**: Usar fallback a regex cuando Gemini falla`);
    }
  }
  ...
  if (conversationHistory.length > 15) {
    ...
    suggestions.push(`💡 **Solución**: Optimizar prompts de Gemini para respuestas más directas`);
  }
  ...
  if (correctionWords.length > 2) {
    ...
    suggestions.push(`💡 **Solución**: Gemini debería detectar automáticamente la corrección final`);
    suggestions.push(`💡 **Mejora**: Revisar prompts de extracción para mejor detección de correcciones`);
  }
  ...
  if (avgResponseLength > 50) {
    ...
    suggestions.push(`💡 **Solución**: Gemini puede extraer múltiples datos de respuestas largas`);
    suggestions.push(`💡 **Mejora**: Prompts más específicos para extracción compleja`);
  }
  
  // 8. ANÁLISIS ESPECÍFICO DE GEMINI
  const geminiErrors = conversationHistory.filter(entry => 
    entry.role === 'bot' && entry.message.includes('Error generando con Gemini')
  );
  if (geminiErrors.length > 0) {
    issues.push(`⚠️ Errores de Gemini detectados (${geminiErrors.length})`);
    suggestions.push(`💡 **Problema**: Fallos en la API de Gemini`);
    suggestions.push(`💡 **Solución**: Verificar API key, límites de uso, y fallbacks`);
  }
  ...
  if (issues.length === 0) {
    ...
    markdown += `- Gemini funcionando perfectamente\n`;
