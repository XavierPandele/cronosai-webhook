...
Idioma:`;
      ...
      if (supportedLangs.includes(detectedLang)) {
        logger.logLanguageDetection(phoneNumber, detectedLang, 0.95, 'gemini');
  ...
  try {
    ...
    try {
      ...
      `, [
        ...
        state.data.phone,
        state.data.name,
        'Reserva por teléfono - Sistema Híbrido',
...
module.exports = async function handler(req, res) {
  ...
  if (userInput) {
    ...
    logger.logIntentAnalysis(From, intentAnalysis, state.step);
  ...
  try {
    ...
    if (intentAnalysis.needs_clarification) {
      ...
      logger.logResponseGeneration(From, response, 'hardcoded', state.language);
    }
  ...
  const metrics = {
    ...
    geminiTime: 0, // No se usa Gemini después de la detección de idioma
  ...
  console.log(`[MÉTRICAS] Sistema: Híbrido (Gemini solo para idioma)`);
